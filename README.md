...
A non-OK status that began while a suppression file hid it stays marked as hidden (`hidden` in the JSON report) until the check's status changes, even after the file is removed. Leaving a hidden status (e.g. the Watchdog or an f3s HTTPS check recovering the morning after an overnight shutdown) is not mailed immediately, since nobody was told about the problem in the first place. Entering CRITICAL from a hidden status still is. A status that stays CRITICAL after the unmute is listed as unhandled and re-notified by the `-renotify` run. A status that was already there when the suppression file appeared is not marked hidden: its CRITICAL may have been mailed, so its recovery is mailed as well.
...
Runs are serialised by an exclusive lock on `StateDir/gogios.lock`, which records the holder's PID: a plain run skips while another run holds it (the running one checks anyway), while `-renotify` and `-force` wait for the lock up to `-lockwait` minutes (default 5), so their mail is not lost. The checks' `-timeout` (minutes, default 5) starts only once the lock is held, so waiting for a slow run does not time checks out. All state and report files are fsynced and replaced atomically, so the web server and the peer always read one complete version. Check output (from plugins, federated instances and the peer) is cleaned up where it enters the state: invalid UTF-8 and control characters other than tab and newline become U+FFFD, ANSI colour sequences are dropped and CR line ends become newlines. Offsetting the `-renotify` and `-force` minutes from the plain runs (e.g. `2 7 * * *`) keeps them from waiting at all.

No run can hold the lock for good: a plugin killed at its timeout is not waited for beyond a few seconds even when a child it forked keeps its output open, the mail delivery times out after 60 seconds, and a watchdog ends the process 2 minutes after `-timeout`. Should the lock still be held for longer than `-timeout` plus 7 minutes, every skipped run fails loudly instead: it prints the stuck PID to stderr, exits 1 and mails a warning (at most once an hour).
...
A passive node mirrors the active peer's check state from the peer's JSON report into its own state and status pages, so both instances publish the same view (the peer's report as of its previous run, i.e. up to one cron interval behind), and a node that takes over starts from recent state. It runs only its host-local checks, marked `"Local": true` (disk, load, swap, ... of the node itself, which only that node can check), and mails only for those (plus everything with `-force`). The active node merges the peer's host-local results from the peer's report (`host` in the JSON report), so both reports list the host checks of both nodes; a check is notified only by the node that ran it. When the peer's report goes stale or cannot be fetched, its host-local checks turn UNKNOWN (they keep their last result in the output), and the active node's own `Gogios peer host checks` check reports it: CRITICAL, and mailed, for a stale report or when the report could not be fetched in two runs in a row (a single failure is UNKNOWN). Host-local check names must differ between the peers (e.g. `Check Disk <hostname>`), and they should not depend on non-local checks.
