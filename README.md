...
Runs are serialised by an exclusive lock on `StateDir/gogios.lock`, which records the holder's PID: a plain run skips while another run holds it (the running one checks anyway), while `-renotify` and `-force` wait for the lock up to `-lockwait` minutes (default 5), so their mail is not lost. The checks' `-timeout` (minutes, default 5) starts only once the lock is held, so waiting for a slow run does not time checks out. All state and report files are replaced atomically. Offsetting the `-renotify` and `-force` minutes from the plain runs (e.g. `2 7 * * *`) keeps them from waiting at all.
